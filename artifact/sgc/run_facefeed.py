...
TARGET_CONFIG_TEMPLATE_PATH = '/experiment/templates/facefeed_config.json.j2'
...
def prep_target(path):
    ...
    config_data = target_config_template.render(path=bin_path, ret_addr=hex(ret_addr), read_seg_str=read_seg_str, write_seg_str=write_seg_str)
...
def verify(entry):
    ...
    for payload_str in payload_list:
        ...
        start = time.time()
        try:
            verify_facefeed(path, payload, known_payload_addr=False)
        ...
        start = time.time()
        try:
            verify_facefeed(path, payload, known_payload_addr=True)
