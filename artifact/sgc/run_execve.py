...
TARGET_CONFIG_TEMPLATE_PATH = '/experiment/templates/execve_config.json.j2'
...
def prep_target(path):
    ...
    try:
        config_data = target_config_template.render(path=bin_path, ret_addr=hex(ret_addr), read_seg_str=read_seg_str, write_seg_str=write_seg_str, syscall_addr=hex(syscall_addr))
    except Exception:
        pass
    print(config_data)
...
def verify(entry):
    ...
    for payload_str in payload_list:
        ...
        start = time.time()
        try:
            verify_execve(path, payload, known_payload_addr=False)
        ...
        start = time.time()
        try:
            verify_execve(path, payload, known_payload_addr=True)
