...
CONFIG_TEMPLATE_PATH = '/experiment/templates/execve_config.toml.j2'
SPEC_PATH = '/experiment/execve.o'
SPEC_SRC_PATH = '/experiment/execve.s'
SPEC_SRC_TEMPLATE_PATH = '/experiment/templates/execve.s.j2'
CONFIG_PATH = "/experiment/config.toml"

with open(CONFIG_TEMPLATE_PATH) as f:
    config_template = Template(f.read())

with open(SPEC_SRC_TEMPLATE_PATH) as f:
    spec_src_template = Template(f.read())
...
def run(path):
    ...
    # create the spec
    e = ELF(path)
    with open(SPEC_SRC_PATH, 'w') as f:
        data = spec_src_template.render(bss_addr=hex(e.bss()))
        f.write(data+'\n')
    os.system(f"gcc -o {SPEC_PATH} -c {SPEC_SRC_PATH}")
