...
from utils import save_tmp_result, do_run, verify_execve
...
def run(path):
    ...
    rop = proj.analyses.ROP(fast_mode=False, max_sym_mem_access=1, cond_br=True)
    try:
    ...
    # clear each gadget's concrete register effect
    for g in rop._all_gadgets:
        g.concrete_regs = {}
    syscall_gadgets = [g for g in rop._all_gadgets if isinstance(g, SyscallGadget)]
    rop._all_gadgets = [g for g in rop._all_gadgets if g.self_contained and len(g.bbl_addrs) <=1 ] + syscall_gadgets
    rop._screen_gadgets()
    ...
    start = time.time()
    try:
        rop.chain_builder._reg_setter.find_candidate_chains_giga_graph_search(None, {'rdi': RopValue(0x41414141, proj), 'rsi': RopValue(0, proj), 'rdx': RopValue(0, proj)}, {}, False)
